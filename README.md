...
| appium:dartVmServicePort | **Requires Flutter ≥3.10.** Pin the Dart VM service to a known port at app launch so the driver doesn't have to scrape the "Dart VM service is listening on …" line from the device log. On **iOS** the driver injects `--vm-service-port=<port>` and `--disable-service-auth-codes` into `processArguments.args`; on **Android** it appends `--ei vm-service-port <port> --ez disable-service-auth-codes true` to `optionalIntentArguments` (Flutter's Android embedding reads these as launch-intent extras). Either way the engine deterministically binds to this port with no auth-code path component. On iOS the value is also used as the fallback URL when the syslog scan exhausts without finding the Dart VM service URL (e.g. when unified-logging privacy filters drop the discovery line). Any pre-existing port flags/extras are stripped and replaced; other entries are left untouched. Does not apply when `observatoryWsUri` is also set (the latter takes precedence for connection). | 8283
...
| -                                                                                                                                  | :ok: | `driver.execute('flutter:getVMServiceUrl')` — the connected Dart VM Service URL (`ws://host:port/[auth-code]/ws`) or `null` | System            |
